...
class App extends Component {
  ...
  adopt = (name) => {
    this.setState(
      (state) => (_.find(this.state.pets, ["name", name]).adopted = true)
    );
  };
        ...
        <main className="container">
          <div className="row">
            <div id="navs" className="col-3">
              <BreedNav
                breeds={Object.keys(_.groupBy(this.state.pets, "breed"))}
              />
              <AboutNav />
            </div>
...
class BreedNav extends Component {
  render() {
    let breedsArr = this.props.breeds.map((breed) => (
      <li key={breed}>
        <a href="">{breed}</a>
      </li>
    ));
    return (
      <nav id="breedLinks">
        <h2>Pick a Breed</h2>
        <ul className="list-unstyled">{breedsArr}</ul>
      </nav>
    );
  }
}
  ...
  render() {
    ...
    return (
      <div
        className="card"
        onClick={() => {
          adoptCallback(petName);
        }}>
        <img className="card-img-top" alt={petName} src={this.props.pet.img} />
        <div className="card-body">
          <h3 className="card-title">{petName}</h3>
          <p className="card-text">
            {this.props.pet.sex + " " + this.props.pet.breed}
          </p>
        </div>
      </div>
    );
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="card-deck">{petList}</div>
      </div>
    );
  }
}
